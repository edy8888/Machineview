...
namespace MatchingModule
{
    ...
    public class MatchingAssistant
	{
        ...
        //‘ˆº”buffer÷¡testimage
        public bool addTestImages(string fileKey, HObject image)
        {
            if (TestImages.ContainsKey(fileKey))
                return false;
            ...
            try
            {
                HImage image0 = new HImage(image);
                TestImages.Add(fileKey, image0);
            }
            catch (HOperatorException e)
            {
                exceptionText = e.Message;
        ...
        public bool saveGreyModel(string fileName)
        {
            if (mImage == null)
            {
                NotifyIconObserver(MatchingAssistant.ERR_NO_MODEL_DEFINED);
            ...
            {
              //  ModelID.WriteShapeModel(fileName);
                ModelIDGrey.WriteTemplate(fileName);
            }
            catch (HOperatorException e)
            {
                exceptionText = e.Message;
        ...
        public bool createGreyModel()
        {
            ...
            try
            {
                ...
                ModelIDGrey = mImage.CreateTemplate(255, 4, "sort", "original");
            }
            catch (HOperatorException e)
            {
